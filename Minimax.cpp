...
class Minimax : public Strategy{
    public :
        tuple<int, int> getValueOfNextState(GameState gameState, int topOfTreeTeamId){
            //default move & utility returned are -1
            int valueOfNextState;
            int moveToNextState;

            //executed checks common to all strategies
            std::tie(valueOfNextState,moveToNextState) = Strategy::getValueOfNextState(gameState, topOfTreeTeamId);
            //if value of or/and move to next state is/are different that default value, these value should be returned (terminal state) 
            if(valueOfNextState != this->defaultValueOfNextState || moveToNextState != this->defaultMoveToNextState)
                ...
                if (gameState.teamId == topOfTreeTeamId){
                    ...
                    for(list<GameState>::iterator it = (gameState.successors).begin(); it != (gameState.successors).end(); it++){
                        ...
                        int successorMinimax = get<0>(getValueOfNextState(*it, topOfTreeTeamId));
